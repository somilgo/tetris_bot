...
let fitness (game : Game.t) : float = 
    ...
    let lines_mult = if lines > 1.0 then 0.760666 else 0. in
    let out = 
        if (int_of_float (compute_height game)) >= (get_height game)
        then (-. max_float) else 
        (remove_piece game;
        draw_piece game;
        ((-. 0.510066) *. height) +.
        ((-. 0.184483) *. compute_bumpy game) +.  
        (lines_mult *. (lines ** 2.0))  +.
        ((-. 0.45663) *. compute_holes game))
    in
    remove_piece game;
    out
...
let make_move (game : Game.t) = 
    ...
    Game.remove_piece (fst best_game); Game.draw_piece (fst best_game);
